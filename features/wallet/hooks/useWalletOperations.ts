...
export function useWalletOperations({
      ...
      try {
        ...
        if (checkedQuote.state === MintQuoteState.PAID) {
          ...
          try {
            ...
            const existingProofs = storedProofs
              ? (JSON.parse(storedProofs) as CashuProof[])
              : [];
            ...
            const newBalance =
              existingProofs.reduce((total, proof) => total + proof.amount, 0) +
            ...
            if (
              ...
              err?.message?.includes("already issued") ||
  ...
  const generateTokenCore = useCallback(
    async (amount: number, mintUrl: string): Promise<string | null> => {
